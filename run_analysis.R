filesPath <- "C:/Proyectos/Coursera/Preparación de los datos"
setwd(filesPath)
...
MeanDeviatOnly <- grep("mean()|std()", features[, 2]) 
dataSet <- dataSet[,MeanStdOnly]

# 4. Appropriately labels the data set with descriptive activity names.

ClnFeatureNames <- sapply(features[, 2], function(x) {gsub("[()]", "",x)})
names(dataSet) <- ClnFeatureNames[MeanDeviatOnly]
